...
跑在 Cloudflare Workers 上的图床，文件存在你自己的 WebDAV 服务上，不需要 R2 绑定。上传走 `/upload`（uPic、curl 都能直接用），访问走 `/i/年/月/日/文件名`。
...
读取接口对所有文件下发 `Content-Security-Policy: default-src 'none'; script-src 'none'; sandbox` 与 `X-Content-Type-Options: nosniff`。这是为了封掉存储型 XSS：后台页面与 `/i/...` 同源，而 Token 就存在浏览器的 localStorage 里，一旦上传的内容能在本站域名下执行脚本，就等于把 Token 交出去。`sandbox` 只在响应被当作**文档打开**时生效，`<img>` / `<video>` / `<audio>` 这类子资源加载完全不受影响，内嵌显示和外站热链都照常工作。

### 上传文件

图片、视频、音频都收，扩展名由 `Content-Type` 或原始文件名推断。
...
**SVG 例外，是允许的**。它是常见图片格式，内嵌到 `<img>` 里本来就不执行脚本。直接用浏览器打开一个含 `<script>` 的 SVG 时，图形照常显示，脚本被 `sandbox` 挡下（控制台会报 `Blocked script execution ... the document's frame is sandboxed`），此时页面处于不透明源，读 `localStorage` 会直接抛 `SecurityError`。
...
客户端把图片转成 Base64 字符串、塞进 JSON 发过来时走这条。
...
- **URL**：`POST /upload` 或 `PUT /i/{year}/{month}/{day}/{filename}`（PUT 可自定义路径）
- **大小上限**：无 20 MB 限制，body 流式透传给 WebDAV，大文件用这种
...
本地调试把域名换成 `http://localhost:8787` 即可，其余参数一样。
...
列出 WebDAV 上 `i/` 目录里的文件。

- **URL**：`GET /admin/list`
- **鉴权**：必须带 `Authorization: Bearer your_secret_token`
- **参数**：
    - `limit`：可选，每次返回的数量，默认 50，最大 100。
    - `cursor`：可选，分页游标，用于取下一页。
    - `year`：可选，按年份筛选，如 `2026`。
    - `month`：可选，按月份筛选，如 `02`（需配合 `year`）。
    - `day`：可选，按日期筛选，如 `25`（需配合 `year` 和 `month`）。
    - `order`：可选，排序方向，`desc` 最新在前（默认）/ `asc` 最早在前。文件名是时间序 ID，所以排序在目录与文件名两级同时生效。换方向后必须从空游标重新开始，沿用旧游标会返回 `无效的分页游标`。
- **示例（curl）**：
...
> 注意：WebDAV 没有快照式分页，某一页可能返回空 `images` 却仍带非空 `cursor`（本轮扫描的目录里恰好没有文件）。调用方要继续沿游标翻页，不能据此判定「没有图片」。
...
参照 [uPic 自定义图床教程](https://blog.svend.cc/upic/tutorials/custom/)，填这几项：
...
5. **URL 路径**：`["url"]`（后端返回的 JSON 结构是 `{"url": "..."}`）
...
`extension/` 目录是一个 Manifest V3 扩展，把网页上的图片存进图床，不需要改 Worker。扩展名为「PhotoFlare 图床助手」，图标、弹窗、设置页和页面通知都沿用管理后台的 logo 与配色（蓝色主操作、红色失败），和后台一样有深色与浅色两套主题，在设置页切换，默认深色。
...
- 结果以通知横幅的形式出现在当前页面一角，默认右上，设置里可改为左上、左下、右下。交互仿 macOS：从所在一侧滑入，先显示「正在上传」，完成后原地变成成功或失败，带「复制链接」「打开」按钮。鼠标悬停时暂停消失，朝所在一侧拖动可关闭。设置里可以关掉，但失败通知始终显示。浏览器内置页、扩展商店这类不允许注入脚本的页面上，改为在工具栏图标上显示 ✓ / ! 角标
...
`POST /mcp` 以 [Model Context Protocol](https://modelcontextprotocol.io/) 暴露一个 `upload_image` 工具，让 Claude Code、Claude Desktop 等 MCP 客户端直接把图片存进本图床并拿到公开 URL，不必再由人转述 curl。
...
支持 JPEG / PNG / GIF / WebP / AVIF / HEIC / BMP / TIFF / ICO，单张不超过 20 MB。**只接受图片**——图床本身支持视频与音频，这个工具刻意不开放它们。**不接受 SVG**：存下来后由本站域名提供服务会构成对后台页面的存储型 XSS。
...
claude.ai 的自定义连接器**不支持填写静态 `Authorization` 头**（那是 beta 白名单功能），只认 OAuth，所以本项目自带一套 OAuth 2.1 授权服务端，不需要额外绑定或依赖。
...
`webdav` 分支用 WebDAV 存储，不需要 R2 绑定。接口路径和管理后台保持兼容。

1. `npm ci` 安装依赖（Node.js 22 或更高版本）。
2. 把 `.dev.vars.example` 复制为 `.dev.vars`，填写 WebDAV 地址、用户名、密码和应用管理 Token。
3. `chmod 600 .dev.vars`。该文件已被 Git 忽略，切勿提交实际凭据。
4. `npm run dev -- --ip 127.0.0.1`，打开 http://127.0.0.1:8787。
...
- `npm test` 验证协议适配、分页、路径和错误处理；`npx wrangler deploy --dry-run` 验证 Worker 打包。
